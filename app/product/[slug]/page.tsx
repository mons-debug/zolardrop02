      ...
      fetch(`/api/products/${slug}`)
        ...
        .then(data => {
          ...
          if (data.product && data.product.variants && data.product.variants.length > 0) {
            ...
            // SECOND PRIORITY: If main product has a color set, don't auto-select any variant
            // This preserves the main product's color (e.g., Pink Fragment) instead of switching to first variant (Bordeaux)
            const mainProductColor = data.product.color?.toLowerCase().trim()
            if (mainProductColor) {
              console.log(`[ProductPage] Main product has color "${data.product.color}" - NOT selecting any variant`)
              // Don't set selectedVariant - let main product display with its own color/images
              setLoading(false)
              return
            }
            ...
            if (variantsWithSizes.length > 0) {
              ...
              if (eclipseBlackVariant) {
                ...
                // Set initial size to first available size but DON'T auto-select a variant color
                setSelectedSize(variantsWithSizes[0].size)
                // Don't auto-select variant - let user pick color
              }
            } else {
              // No sizes, prioritize Eclipse Black for essence products
              // Otherwise DON'T auto-select - user should pick
              if (eclipseBlackVariant) {
                setSelectedVariant(eclipseBlackVariant)
              }
              // Removed: auto-selecting first variant - this was causing Pink Fragment to show Bordeaux
            }
          }
          setLoading(false)
        })
  ...
  const handleAddToCart = () => {
    ...
    if (hasSizes && selectedSize) {
      ...
      if (displaySizeInventory.length > 0) {
        ...
        // - If selectedVariant is null (main product color): use product.images, NOT variant images
        let cartImage: string
        if (selectedVariant) {
          const selectedVariantImgs = parseImages(selectedVariant.images)
          cartImage = selectedVariantImgs[0] || firstImage
        } else {
          // Main product color selected - use product's own images
          const productImgs = parseImages(product.images)
          cartImage = productImgs[0] || firstImage
        }

        // CRITICAL FIX: Use correct color name
        // - If selectedVariant exists: use selectedVariant.color
        // - If selectedVariant is null: use product.color (the main product color user clicked)
        const displayColorName = selectedVariant ? selectedVariant.color :
          (product.color || 'Default')

        console.log('Adding to cart (sizeInventory):', {
          isMainProductColor: !selectedVariant,
          selectedVariantId: selectedVariant?.id,
          selectedVariantColor: selectedVariant?.color,
          productColor: product.color,
          finalDisplayColor: displayColorName,
          imageUsed: selectedVariant ? 'variant image' : 'product image',
          selectedSize
        })
        ...
        if (selectedVariant) {
          ...
          cartImage = variantImgs[0] || firstImage
        } else {
          // Main product color selected - use product's own images
          const productImgs = parseImages(product.images)
          cartImage = productImgs[0] || firstImage
        ...
        // IMPORTANT: Use correct color name
        // - If selectedVariant exists: use selectedVariant.color
        // - If selectedVariant is null: use product.color
        const displayColorName = selectedVariant ? selectedVariant.color :
          (product.color || variantForSize.color || 'Default')

        console.log('Adding to cart (variant-sizes):', {
          isMainProductColor: !selectedVariant,
          selectedVariantColor: selectedVariant?.color,
          productColor: product.color,
          variantForSizeColor: variantForSize.color,
          finalDisplayColor: displayColorName,
          imageUsed: selectedVariant ? 'variant image' : 'product image'
        })
        ...
        addItem({
          productId: product.id,
          variantId: selectedVariant?.id || variantForSize.id,
          qty: qty,
          priceCents: selectedVariant?.priceCents || variantForSize.priceCents,
          title: product.title,
          image: cartImage,
          variantName: `${displayColorName} - ${selectedSize}`,
          size: selectedSize
                  ...
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2 z-10">
                    {allImages.map((_, index) => (
                      <button
                        ...
                        className={`transition-all ${index === currentImageIndex ? 'bg-black w-8 h-0.5' : 'bg-gray-300 w-8 h-0.5'
                          }`}
                      />
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {allImages.map((image, index) => (
                    <button
                      ...
                      className={`aspect-[3/4] relative bg-gray-50 border transition-all ${index === currentImageIndex ? 'border-black' : 'border-gray-200 hover:border-gray-400'
                        }`}
                    >
                      <Image
                        src={image}
            ...
            <div className="space-y-8">
                    ...
                    {displaySizeInventory.map(({ size, quantity }) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-2.5 text-xs uppercase tracking-wider font-medium border-2 transition-all rounded-sm ${selectedSize === size
                      ...
                      <div className="flex items-center gap-4 mb-6 p-3 bg-gray-50 border border-gray-200 rounded-sm">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`text-xs font-medium ml-2 ${isLowStock ? 'text-orange-600' : 'text-green-600'
                              }`}>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {getAllColors().map((colorOption: any) => (
                      <button
                        ...
                        className={`px-5 py-3 text-xs uppercase tracking-wider font-medium border-2 transition-all rounded-sm ${(colorOption.isMainProduct && !selectedVariant) ||
              ...
              <div className="space-y-4">
                <button
                  ...
                  className={`w-full py-4 px-8 text-xs uppercase tracking-widest font-medium transition-colors ${((displaySizeInventory.length > 0 || getAvailableSizes().length > 0) && (!selectedSize || !sizeQuantities[selectedSize]))
